...
<?php if (!@$htmlOptions['nowrapper']) {
    ?>
	<div id="<?php echo $div_id ?>" class="<?php echo $div_class ?> row field-row widget"<?php if ($hidden) {
    ...
    echo ' multi-select-list';
			...
			<div class="multi-select-dropdown-container">
				<select id="<?php echo CHtml::getIdByName($field) ?>"
						class="MultiSelectList<?php if ($showRemoveAllLink) {
    ?> inline<?php 
    ...
    ?>
					<a href="#" class="remove-all<?php echo !$found ? ' hide': '';
    ?>">Remove all</a>
				<?php 
}?>
			</div>
    ...
    ?>
				<div class="no-selections-msg pill<?php if ($found) {
    ?> hide<?php 
}
    ?>"><?php echo $noSelectionsMessage;
    ?></div>
			<?php 
}?>
    ...
    if (isset($options[$id])) {
        ?>
						<li>
							...
							<a href="#" data-text="<?php echo $options[$id] ?>" class="MultiSelectRemove remove-one<?php if (isset($htmlOptions['class'])) {
    ?> <?php echo $htmlOptions['class']?><?php 
}
        ?>"<?php if (isset($htmlOptions['data-linked-fields'])) {
