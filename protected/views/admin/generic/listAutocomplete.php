        ...
        if (is_array($admin->getFilterFields())) {
            foreach ($admin->getFilterFields() as $field => $params) {
				...
				<div class="large-2 column"><label
						for="<?php echo $params['dropDownName'] ?>"><?php echo $params['label'];
                ?>:</label></div>
		...
		<div class="row field-row">
			<table class="grid">
                ...
                foreach ($admin->getSearch()->retrieveResults() as $i => $row) {
                    ...
                    ?>','<?php echo $admin->getCustomDeleteURL();
                    ?>')">Delete</a>
							<?php if ($listItem == "default") {
    ?>
								&nbsp;&nbsp;|&nbsp;&nbsp;<a OnCLick="setDefaultItem('<?php echo $row->id;
    ?>','<?php echo $admin->getCustomSetDefaultURL();
    ?>')">Set Default</a>
								&nbsp;&nbsp;|&nbsp;&nbsp;<a OnCLick="removeDefaultItem('<?php echo $row->id;
    ?>','<?php echo $admin->getCustomRemoveDefaultURL();
    ?>')">Remove Default</a>
						<?php 
}
                    ?>
